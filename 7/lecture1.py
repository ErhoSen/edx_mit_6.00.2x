class puzzle(object):
    ...
    def transition(self, to): # to = 3
        label = self.label # lavel = 125638047
        blankLocation = self.spot # blankLocation = 6 // position of 0
        newBlankLabel = str(label[to]) # newBlankLabel = '6'
        newLabel = ''
        for i in range(9):
            if i == to:
                newLabel += '0'
